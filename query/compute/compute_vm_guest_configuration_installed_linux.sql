...
vm_guest_configuration as (
  ...
  from
    all_linux_vm as a left join vm_extensions as b on  (b.arguments ->> 'virtual_machine_id') = a.id
  where
...
select
  ...
  path
from
  all_linux_vm as c left join vm_guest_configuration as d on c.id = d.vm_id;